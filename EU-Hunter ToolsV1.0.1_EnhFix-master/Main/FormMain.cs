...
#pragma warning disable CS0414 // Полю "FormMain.nombre" присвоено значение, но оно ни разу не использовано.
        private string nombre = "";
#pragma warning restore CS0414 // Полю "FormMain.nombre" присвоено значение, но оно ни разу не использовано.
...
#pragma warning disable CS0414 // Полю "FormMain.teamWait" присвоено значение, но оно ни разу не использовано.
        private bool teamWait = false;
#pragma warning restore CS0414 // Полю "FormMain.teamWait" присвоено значение, но оно ни разу не использовано.
...
#pragma warning restore CS0168 // Переменная "ex" объявлена, но ни разу не использована.
       ...
       "Reset all data", MessageBoxButtons.YesNo);
        ...
        private void analyseLine(string line)
        {
            ...
            try
            {
                ...
                if (line.Contains("[System] [] You received"))// тут изменяется лут                
                {
                    ...
                    //if mining mode on and Emode of current prof off, add decay (made for excovators track) (and looted item isnt in unused list)
                    if (this.STT.MiningMode.Checked && (!this.fCC.ProfList[Convert.ToInt32(ProfTetboxHelp) - 1].USEEFLAG) && !globalFlagUnused)
                    {
                        SecondsBeforeAFKTimer = 0;
                        this.ammoSpent += this.ammoPerShot;
                        AddItemStatus(0);
        ...
        {
#pragma warning disable CS0219 // Переменной "flagNotUsual" присвоено значение, но оно ни разу не использовано.
            bool flagNotUsual = false;
#pragma warning restore CS0219 // Переменной "flagNotUsual" присвоено значение, но оно ни разу не использовано.
            string item;
            string value;
            ...
            else
            {
                ...
                else if (lineSplit.Contains("found something"))
                    LineForSwitch = "found something";
            ...
            switch (LineForSwitch)
            {
                ...
                case "found something": //dungeon loot
                    item = line.Remove(0, line.IndexOf(" examined") + 10);
                    item = item.Remove(item.LastIndexOf(" and found"));
                    value = line.Remove(0, line.IndexOf("a value of") + 11);
                    value = value.Remove(value.IndexOf(" "));
                    rows = dGVGlobals.RowCount;
                    dGVGlobals.Rows.Add();
        ...
        private void AddEnhancer(List<EnhItems> listEnh, string line, bool _isMain)
        {
            try
            {
                ...
                //    EnhName = EnhName.Remove(EnhName.IndexOf("Enhancer") + 10, (EnhName.IndexOf("Shrapnel") + 9) - (EnhName.IndexOf("Enhancer") + 10));
                //else
                EnhName = EnhName.Remove(EnhName.IndexOf("Enhancer") + 8, EnhName.Length - (EnhName.IndexOf("Enhancer") + 8));  
                
                EnhName = EnhName.Remove(0, 14);             
                EnhTTstr = line.Remove(0, line.IndexOf("received") + 9);
                EnhTTstr = EnhTTstr.Remove(EnhTTstr.IndexOf(" "), EnhTTstr.Length - EnhTTstr.IndexOf(" "));
